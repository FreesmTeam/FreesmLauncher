...
MinecraftAccountPtr MinecraftAccount::createCustom(const QString& login,
...
shared_qobject_ptr<AuthFlow> MinecraftAccount::login(QString password)
{
    Q_ASSERT(m_currentTask.get() == nullptr);
...
void MinecraftAccount::fillSession(AuthSessionPtr session, SettingsObject* settings)
{
    // volatile auth token
