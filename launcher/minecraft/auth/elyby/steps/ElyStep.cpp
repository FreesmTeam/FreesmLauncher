...
ElyStep::ElyStep(AccountData* data, bool silent) : AuthStep(data), m_silent(silent)
{
    ...
    connect(&m_oauth2, &QOAuth2AuthorizationCodeFlow::granted, this, [this] {
        m_data->clientID = m_oauth2.clientIdentifier();
...
void ElyStep::perform()
{
    if (m_silent) {
        if (m_data->clientID != m_clientId) {
            emit finished(AccountTaskState::STATE_DISABLED, tr("Ely.by user authentication failed - client identification has changed."));
            return;
        }
